...
2. **Run audit first** — `/gsdr:audit-milestone` to assess gap severity
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" summary-extract "$summary" --fields one_liner | jq -r '.one_liner'
done
```
...
```bash
ARCHIVE=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" milestone complete "v[X.Y]" --name "[Milestone Name]")
```
...
If "Skip": Phase directories remain in `.planning/phases/` as raw execution history. Use `/gsdr:cleanup` later to archive retroactively.
...
**If doesn't exist:** Create from template at `${CLAUDE_SKILL_DIR}/../templates/retrospective.md`.
...
**Commit:**
```bash
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" commit "docs: update retrospective for v${VERSION}" --files .planning/RETROSPECTIVE.md
```
...
```bash
INIT=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" init execute-phase "1")
...
```bash
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" commit "chore: complete v[X.Y] milestone" --files .planning/milestones/v[X.Y]-ROADMAP.md .planning/milestones/v[X.Y]-REQUIREMENTS.md .planning/milestones/v[X.Y]-MILESTONE-AUDIT.md .planning/MILESTONES.md .planning/PROJECT.md .planning/STATE.md
```
```
...
- [ ] User knows next step (/gsdr:new-milestone)
