...
<purpose>
Create all phases necessary to close gaps identified by `/gsdr:audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/gsdr:add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/gsdr:audit-milestone` first.
```
...
PHASES=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" phases list)
...
```bash
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
...
**Also available:**
- `/gsdr:execute-phase {N}` — if plans already exist
...
`/gsdr:audit-milestone` — re-audit to verify gaps closed
`/gsdr:complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/gsdr:plan-phase` next
</success_criteria>
