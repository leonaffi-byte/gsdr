...
```bash
INIT=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" init progress)
...
- `STATE=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" progress bar --raw)
```
...
- `/gsdr:list-phase-assumptions {phase}` — see Claude's assumptions
...
**Also available:**
- `/gsdr:execute-phase {phase}` — execute phase plans
...
**Also available:**
- `/gsdr:verify-work` — user acceptance test before completing milestone
...
- Handoff file exists → mention it, offer `/gsdr:resume-work`
  </step>
...
- [ ] Smart routing: /gsdr:execute-phase if plans exist, /gsdr:plan-phase if not
