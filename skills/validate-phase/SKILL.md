...
```bash
AUDITOR_MODEL=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" resolve-model gsdr-nyquist-auditor --raw)
...
## 5. Spawn gsdr-nyquist-auditor

```
Task(
  prompt="Read ${CLAUDE_SKILL_DIR}/../agents/gsdr-nyquist-auditor.md for instructions.\n\n" +
    "<files_to_read>{PLAN, SUMMARY, impl files, VALIDATION.md}</files_to_read>" +
    "<gaps>{gap list}</gaps>" +
    "<test_infrastructure>{framework, config, commands}</test_infrastructure>" +
    "<constraints>Never modify impl files. Max 3 debug iterations. Escalate impl bugs.</constraints>",
  subagent_type="gsdr-nyquist-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `${CLAUDE_SKILL_DIR}/../templates/VALIDATION.md`
...
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" commit-docs "docs(phase-${PHASE}): add/update validation strategy"
```
