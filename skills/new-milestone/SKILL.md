...
- Check for MILESTONE-CONTEXT.md (from /gsdr:discuss-milestone)
...
```bash
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" commit "docs: start milestone v[X.Y] [Name]" --files .planning/PROJECT.md .planning/STATE.md
```

## 7. Load Context and Resolve Models

```bash
INIT=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" init new-milestone)
...
**Persist choice to config** (so future `/gsdr:plan-phase` honors it):

```bash
# If "Research first": persist true
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" config-set workflow.research true

# If "Skip research": persist false
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" config-set workflow.research false
```
...
Spawn 4 parallel gsdr-project-researcher agents. Each uses this template with dimension-specific fields:
...
Use template: ${CLAUDE_SKILL_DIR}/../templates/research-project/{FILE}
</output>
", subagent_type="gsdr-project-researcher", model="{researcher_model}", description="{DIMENSION} research")
```
...
**Commit requirements:**
```bash
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" commit "docs: define milestone v[X.Y] requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" commit "docs: create milestone v[X.Y] roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md
```
...
`/gsdr:discuss-phase [N]` — gather context and clarify approach

<sub>`/clear` first → fresh context window</sub>

Also: `/gsdr:plan-phase [N]` — skip discussion, plan directly
```
...
- [ ] gsdr-roadmapper spawned with phase numbering context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] ROADMAP.md phases continue from previous milestone
- [ ] All commits made (if planning docs committed)
- [ ] User knows next step: `/gsdr:discuss-phase [N]`
