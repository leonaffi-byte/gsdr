...
context: fork
agent: gsdr-integration-checker
...
```bash
INIT=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" init milestone-op)
...
```bash
integration_checker_model=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" resolve-model gsdr-integration-checker --raw)
```
...
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" phases list
```
...
PHASE_INFO=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" find-phase 01 --raw)
...
Verify cross-phase wiring and E2E user flows.",
  subagent_type="gsdr-integration-checker",
...
for summary in .planning/phases/*-*/*-SUMMARY.md; do
  node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" summary-extract "$summary" --fields requirements_completed | jq -r '.requirements_completed'
done
```
...
```bash
NYQUIST_CONFIG=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" config get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/gsdr:validate-phase`.
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/gsdr:validate-phase {N}` |

Phases needing validation: run `/gsdr:validate-phase {N}` for each flagged phase.
...
- /gsdr:complete-milestone {version} — proceed anyway (accept tech debt)
