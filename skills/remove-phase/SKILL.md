...
- Example: `/gsdr:remove-phase 17` → phase = 17
- Example: `/gsdr:remove-phase 16.1` → phase = 16.1

If no argument provided:

```
ERROR: Phase number required
Usage: /gsdr:remove-phase <phase-number>
Example: /gsdr:remove-phase 17
```
...
```bash
INIT=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" init phase-op "${target}")
...
To abandon current work, use /gsdr:pause-work instead.
```
...
```bash
RESULT=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" phase remove "${target}")
```

If the phase has executed plans (SUMMARY.md files), gsd-tools will error. Use `--force` only if the user confirms:

```bash
RESULT=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" phase remove "${target}" --force)
```
...
```bash
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" commit "chore: remove phase {target} ({original-phase-name})" --files .planning/
```
...
Would you like to:
- `/gsdr:progress` — see updated roadmap status
