...
```bash
timestamp=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" current-timestamp full --raw)
```
</step>

<step name="commit">
```bash
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md
```
</step>
