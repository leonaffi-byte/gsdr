...
<purpose>
Execute small, ad-hoc tasks with GSDR guarantees (atomic commits, STATE.md tracking). Quick mode spawns gsdr-planner (quick mode) + gsdr-executor(s), tracks tasks in `.planning/quick/`, and updates STATE.md's "Quick Tasks Completed" table.
...
```bash
INIT=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" init quick "$DESCRIPTION")
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/gsdr:new-project` first.
...
Spawn gsdr-executor with plan reference:
...
```bash
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
...
Ready for next task: /gsdr:quick
```
