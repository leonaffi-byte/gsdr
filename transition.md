...
Skipping incomplete plans is destructive -- ALWAYS prompt regardless of auto-advance setting.
...
**Delegate ROADMAP.md and STATE.md updates to gsdr-tools:**

```bash
TRANSITION=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" phase complete "${current_phase}")
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `gsdr-tools phase complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" progress bar --raw)
```
...
**Use the transition result from `gsdr-tools phase complete`:**
...
```bash
ROADMAP=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" roadmap analyze)
```
...
**Auto-advance path** (when `AUTO_CHAIN` or `AUTO_CFG` is `"true"`):
...
Invoke `/gsdr:plan-phase [X+1] --auto`
...
Invoke `/gsdr:discuss-phase [X+1] --auto`

**Non-auto path** (when neither `AUTO_CHAIN` nor `AUTO_CFG` is `"true"`):
...
`/gsdr:discuss-phase [X+1]` -- gather context and clarify approach

---

**Also available:**
- `/gsdr:plan-phase [X+1]` -- skip discussion, plan directly
- `/gsdr:research-phase [X+1]` -- investigate unknowns
...
**Also available:**
- `/gsdr:discuss-phase [X+1]` -- revisit context
- `/gsdr:research-phase [X+1]` -- investigate unknowns
...
```bash
node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" config-set workflow._auto_chain_active false
```
...
Invoke `/gsdr:complete-milestone {version}`
