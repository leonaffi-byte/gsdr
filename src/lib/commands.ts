...
export function cmdHistoryDigest(cwd: string, raw: boolean): void {
  ...
  const digest: {
    phases: Record<string, PhaseDigest>;
  ...
  try {
      ...
      for (const summary of summaries) {
        try {
          ...
          if (depGraph?.provides) {
            (depGraph.provides as string[]).forEach((p: string) => digest.phases[phaseNum].provides.add(p));
          } else if (fm.provides) {
            (fm.provides as string[]).forEach((p: string) => digest.phases[phaseNum].provides.add(p));
          }

          if (depGraph?.affects) {
            (depGraph.affects as string[]).forEach((a: string) => digest.phases[phaseNum].affects.add(a));
          }

          if (fm['patterns-established']) {
            (fm['patterns-established'] as string[]).forEach((p: string) => digest.phases[phaseNum].patterns.add(p));
          }
          ...
          if (techStack?.added) {
            (techStack.added as Array<string | { name: string }>).forEach(t => digest.tech_stack.add(typeof t === 'string' ? t : t.name));
          }
    ...
    const outputPhases: Record<string, { name: string; provides: string[]; affects: string[]; patterns: string[] }> = {};
    for (const [key, val] of Object.entries(digest.phases)) {
      outputPhases[key] = {
        name: val.name,
    ...
    output({ phases: outputPhases, decisions: digest.decisions, tech_stack: [...digest.tech_stack] }, raw);
  } catch (e: unknown) {
...
export function cmdCommit(cwd: string, message: string | undefined, files: string[], raw: boolean, amend?: boolean): void {
  if (!message && !amend) {
...
export function cmdSummaryExtract(cwd: string, summaryPath: string, fields: string[] | null, raw: boolean): void {
  if (!summaryPath) {
...
export function cmdScaffold(cwd: string, type: string, options: ScaffoldOptions, raw: boolean): void {
  ...
  switch (type) {
    case 'context': {
      ...
      content = `---\nphase: "${padded}"\nname: "${name || phaseInfo?.phase_name || 'Unnamed'}"\ncreated: ${today}\n---\n\n# Phase ${phase}: ${name || phaseInfo?.phase_name || 'Unnamed'} -- Context\n\n## Decisions\n\n_Decisions will be captured during /gsdr:discuss-phase ${phase}_\n\n## Discretion Areas\n\n_Areas where the executor can use judgment_\n\n## Deferred Ideas\n\n_Ideas to consider later_\n`;
      break;
    }
    case 'uat': {
