...
```bash
PHASE_INFO=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" roadmap get-phase "${PHASE}")
...
```bash
PHASE_DIR=$(node "${CLAUDE_SKILL_DIR}/../dist/gsdr-tools.cjs" find-phase "${PHASE}" --raw)
```
